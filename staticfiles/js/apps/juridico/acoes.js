...
// Nova função para visualizar dados da ação
function visualizarDadosAcao(acaoId) {
    $.get(`/juridico/api_get_acao/${acaoId}/`, function(response) {
        if (response.success) {
            ...
            $('#modalVerDadosAcaoLabel').text(`Detalhes da Ação #${acao.id}`);
            $('#dadosClienteNome').text(acao.nome_cliente || 'N/A');
            $('#dadosClienteCPF').text(acao.cpf_cliente ? formatarCPF(acao.cpf_cliente) : 'N/A');
            $('#dadosTipoAcao').text(acao.tipo_acao || 'N/A'); // Ajustar para pegar o display name se necessário
            $('#dadosDataCriacao').text(acao.data_criacao || 'N/A');
            $('#dadosStatusAcao').text(acao.status_emcaminhamento || 'N/A'); // Ajustar para pegar o display name
            $('#dadosSentenca').text(acao.sentenca || 'N/A'); // Ajustar para pegar o display name
            $('#dadosLoja').text(acao.loja || 'N/A');
            $('#dadosResponsavel').text(acao.responsavel_nome || 'N/A');
            $('#dadosSenhaINSS').text(acao.senha_inss || 'N/A');

            // Preencher tabela de documentos/arquivos da ação
            const tbodyArquivos = $('#tabelaArquivosDadosAcao');
            tbodyArquivos.empty();
            if (acao.documentos_acao && acao.documentos_acao.length > 0) {
                $('#nenhumArquivoDadosAcao').hide();
                acao.documentos_acao.forEach(arquivo => {
                    tbodyArquivos.append(`
                        <tr>
                            <td>${arquivo.titulo}</td>
                            <td>${new Date(arquivo.data_import).toLocaleDateString()}</td>
                            <td class="text-center">
                                ...
                                <button class="btn btn-primary btn-sm" onclick="baixarArquivoPelaApi(${arquivo.id})" data-tooltip="Baixar">
                                    <i class='bx bx-download'></i>
                                </button>
                            </td>
                        </tr>
