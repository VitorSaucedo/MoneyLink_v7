...
$(document).ready(function() {
    ...
    function moverCard(tabulacaoId, novoStatus, cardElement) {
        ...
        if (window.kanbanPermissoes && !window.kanbanPermissoes.pode_ver_todos) {
            console.log('Movendo card com permissões restritas');
        }
    ...
    function renderizarDetalhesModal(dados) {
        ...
        // Renderização dos arquivos de negociação para REVERSAO e CHECAGEM
        let arquivosNegociacaoHtml = '';
        if ((tabulacao.status === 'REVERSAO' || tabulacao.status === 'CHECAGEM') && dados_negociacao && Array.isArray(dados_negociacao.arquivos) && dados_negociacao.arquivos.length > 0) {
            arquivosNegociacaoHtml = `
            <div class="${tabulacao.status === 'REVERSAO' ? 'secao-reversao' : 'secao-negociacao'}">
                <div class="detalhe-secao">
                    <h6 class="detalhe-titulo">
                        <i class="fas fa-paperclip"></i>
                        Arquivos ${tabulacao.status === 'REVERSAO' ? 'da Reversão' : 'da Negociação'}
                    </h6>
        ...
        // Dados de negociação - exibir apenas para status CHECAGEM
        const dadosNegociacaoHtml = (tabulacao.status === 'CHECAGEM' && dados_negociacao && dados_negociacao.existe) ? `
        ...
        const modalContent = `
            ...
            ${((tabulacao.status === 'REVERSAO' || tabulacao.status === 'CHECAGEM') && documentos.length > 0) ? `
            <div class="${tabulacao.status === 'REVERSAO' ? 'secao-reversao' : 'secao-documentos'}">
                <div class="detalhe-secao">
                    <h6 class="detalhe-titulo">
                        <i class="fas fa-file-alt"></i>
                        ${tabulacao.status === 'REVERSAO' ? 'Arquivos da Reversão' : 'Arquivos da Negociação'}
                    </h6>
                    <div class="card-content-wrap">
                        ${documentosHtml}
                    </div>
    ...
    function ajustarAlturasColunas() {
        ...
        $('.kanban-column').each(function() {
            ...
            if (numeroCards === 0) {
                ...
                const alturaCard = 140; // Altura aproximada de cada card incluindo margens
                const padding = 12; // Padding da coluna (--spacing-sm * 2)
                const alturaTotal = (numeroCards * alturaCard) + padding;
                
                $containerCards.css({
                    'max-height': `${Math.max(alturaTotal, 150)}px`,
                    'min-height': '150px',
                    'overflow-y': 'visible'
                });
                
                console.log(`Coluna com ${numeroCards} cards: altura ${alturaTotal}px (sem scroll)`);
            } else {
                // Se tem mais cards que o máximo, fixa altura e ativa scroll
                const alturaMax = (maxCardsVisiveis * 140) + 12;
                ...
                console.log(`Coluna com ${numeroCards} cards: altura fixa ${alturaMax}px (com scroll para ${numeroCards - maxCardsVisiveis} cards ocultos)`);
