...
def api_get_dashboard_agendamentos_overview(request):
    ...
    Permite filtros por data_inicio, data_fim, funcionario_id e status.
    """
    ...
    try:
        ...
        # Query base de agendamentos - incluir relacionamento com funcionário se existe
        ...
        if status_filtro:
            agendamentos_qs = agendamentos_qs.filter(status=status_filtro)
        ...
        hoje = timezone.now().date()
        agendamentos_hoje = agendamentos_qs.filter(data=hoje).count()
        
        # Calcular taxa de conversão (fechamentos / total de agendamentos não pendentes)
        agendamentos_finalizados = agendamentos_qs.exclude(status='AGENDADO').count()
        total_fechamentos = agendamentos_qs.filter(status='FECHOU').count()
        
        if agendamentos_finalizados > 0:
            taxa_conversao = round((total_fechamentos / agendamentos_finalizados) * 100, 1)
        else:
            taxa_conversao = 0.0
        ...
        status_counts = {}
        for choice in AgendamentoFichaCliente.STATUS_CHOICES:
            status_key = choice[0]
            status_label = choice[1]
            count = agendamentos_qs.filter(status=status_key).count()
            status_counts[status_label] = count

        # Timeline dos últimos 30 dias
        data_limite = hoje - timedelta(days=30)
        agendamentos_timeline = agendamentos_qs.filter(data__gte=data_limite)
        
        timeline_data = {}
        for i in range(30):
            data_atual = hoje - timedelta(days=i)
            count = agendamentos_timeline.filter(data=data_atual).count()
            timeline_data[data_atual.strftime('%Y-%m-%d')] = count

        # Top 10 funcionários com mais agendamentos
        funcionarios_stats = agendamentos_qs.values('usuario__username', 'usuario_id').annotate(
            total=Count('id'),
            fechamentos=Count('id', filter=Q(status='FECHOU'))
        ).order_by('-total')[:10]
        ...
        for stat in funcionarios_stats:
            ...
            funcionarios_data.append({
                'nome': nome,
                'total_agendamentos': stat['total'],
                'total_fechamentos': stat['fechamentos']
            })

        # Horários mais agendados
        horarios_stats = agendamentos_qs.extra(
...
def api_get_agendamentos_detalhes(request):
    ...
    try:
        ...
        for agendamento in agendamentos_pagina:
            ...
            status_display = dict(AgendamentoFichaCliente.STATUS_CHOICES).get(agendamento.status, agendamento.status)
...
def api_get_detalhes_card_kanban(request):
    ...
    try:
        ...
        try:
            ...
            # Buscar arquivos se for REVERSAO ou CHECAGEM
            if dados_negociacao and tabulacao.status in ['REVERSAO', 'CHECAGEM']:
                arquivos = ArquivoNegociacao.objects.filter(
                    dados_negociacao=dados_negociacao,
                    status=True
                ).order_by('-data_criacao')
