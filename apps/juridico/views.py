...
def api_upload_arquivo_acao(request):
    """
    API para adicionar um novo arquivo à ação (ArquivosAcoesINSS).
    """
    try:
        acao_id = request.POST.get('acao_id')
        titulo = request.POST.get('titulo')
        ...
        return JsonResponse({'status': 'success', 'message': 'Arquivo enviado com sucesso!', 'arquivo_id': arquivo.id})
