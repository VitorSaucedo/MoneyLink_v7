...
function enviarArquivo() {
    console.log('[ENVIAR_ARQUIVO] Iniciando envio de arquivo...');
    
    const formData = new FormData();
    const acaoId = $('#acaoIdArquivos').val();
    const titulo = $('#tituloArquivo').val();
    const arquivo = $('#arquivo')[0].files[0];
    
    if (!titulo || !arquivo) {
        mostrarErro('Título e arquivo são obrigatórios.');
        return;
    }
    ...
    $.ajax({
        ...
        contentType: false,
        success: function(response) {
            console.log('[ENVIAR_ARQUIVO] Resposta recebida:', response);
            if (response.status === 'success') {
                mostrarSucesso('Arquivo enviado com sucesso!');
                $('#modalAdicionarArquivos').modal('hide');
                // Recarregar a página para atualizar a lista
                location.reload();
            } else {
                mostrarErro('Erro ao enviar arquivo: ' + (response.message || 'Erro desconhecido'));
            }
        },
        error: function(xhr, status, error) {
            console.error('[ENVIAR_ARQUIVO] Erro na requisição:', {xhr, status, error});
            mostrarErro('Erro ao enviar arquivo. Tente novamente.');
        },
        complete: function() {
            // Reabilitar botão
